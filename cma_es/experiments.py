...
class Experiments:
    ...
    def log_metrics_and_data(self):
        # data
        data_dict = {'true_D': self.true_D, 'true_B': self.true_B, 'data': self.data, 'data_cov': self.data_cov, 'pred_D': self.pred_D, 'pred_B': self.pred_B, 'pred_thresh_D': self.pred_thresh_D if (self.do_thresholding and self.algorithm_name != 'GFCI') else None, 'pred_thresh_B': self.pred_thresh_B if (self.do_thresholding and self.algorithm_name != 'GFCI') else None, 'captured_metrics': self.captured_metrics}
...
def run_sachs_dataset():
    for algo in ['DCD', 'CMA-ES', 'Relcadilac']:
        ...
        exp.algorithm = exp.get_algorithm()
        exp.explanation = f"Run {exp.run_number}; {exp.algorithm_name}; Demo"
        exp.run_test()

def run_demo():
    for algo in ['DCD', 'CMA-ES', 'Relcadilac', 'GFCI']:
        exp = Experiments()
        exp.num_nodes = 5
        exp.sample_size = 500
        exp.algorithm_name = algo
        exp.algorithm = exp.get_algorithm()
        exp.steps_per_env = 500
        exp.n_envs = 3
        exp.max_fevals = 1000
        exp.explanation = f"Run {exp.run_number}; {exp.algorithm_name}; Demo"
        exp.run_test()
